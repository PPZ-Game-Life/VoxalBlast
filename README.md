...
> 当前实现基线：**v0.8.21**（`package.json`）。本文按源码整理；不代表远端部署版本或本轮运行验收结果。

## 当前游戏

- **六面共享棋盘**：5×5×5 外壳，98 个唯一格；每面 5×5，棱角在相邻面共享，不往内部填块，无重力。
- **开场创建波次**：进入一局时六面体不是瞬间完整出现，而是被一道斜向（屏幕左下 → 右上）波次逐格创建出来，约 1.05 秒；彩色已占用格块稍晚出现并有一次轻微亮度提升。参数在 `INTRO_STYLE`，`prefers-reduced-motion` 时退化为 0.18 秒整盘淡入。
...
npm run probe:intro    # 开场创建波次（波向、节奏、结束位姿，并录下开场帧序）
