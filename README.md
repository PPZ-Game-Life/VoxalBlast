...
当前版本 **0.8.8**：停稳角度改成玩家可微调的**方位**——阈值（≈30°）以内的拖动**保留**为新的停稳角度（不回弹、不二次动画），翻面时**记住**这个方位，新的一面按它停下；"方位 + 本次拖动"越出 ±30° 才翻面。平面自转不留残角，方位随续玩存档保存。停稳构图（出厂方位）主面占三个可见面投影面积 71.4%（24 种标准朝向零离散），六面体竖直棱恒为铅直。玩法、输入映射与阈值沿用 v0.8.7。口径见 [03 §2.1](docs/Planning/03-交互与表现需求.md)，验收见 [04](docs/Planning/04-MVP验收清单.md)。
