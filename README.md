...
当前版本 **0.8.10**：停稳时网格不再有屏幕面内旋转。相机换成**弱透视 + 近乎水平**的 3/4 视角（`cameraFov 30→6`、俯仰 17.6°→8°），正面网格线的屏幕偏斜从 **13.5° 降到 3.0° 以内**（24 朝向，v0.8.9 是 −3.7°…13.5°），竖直棱仍 0.00°。附带把面内滚转离散 7.66°→2.27°、俯仰轴偏斜 5.98°→2.9°。构图：主面 74.3% / 侧面 18.0% / 顶面 7.8%。代价是透视变平（观感接近正交）、顶面变薄。玩法、输入映射与阈值沿用 v0.8.9。
