...
第六轮补测了**真正不删形状**的加权发牌（此前把 `w90` 说成"不删形状"是错的——它的 Dot/Line 2/Corner 权重为 0，与 `d` 组成员相同）：十种全留、只把四格件权重调高，`soft75`/`soft82` 把典型玩家结束率从 1.0% 抬到 **4.2%/8.8%**、最紧时刻 9 → 6/5 个落点；小步可调、可回滚。
