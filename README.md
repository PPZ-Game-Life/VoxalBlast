...
当前版本 **0.8.7**：停稳构图与旋转观感——3/4 视角回到**相机**（`cameraDirection` 由 v0.8.5 的 26.4°/22.3° 收到约 18.8°/17.6°），六面体竖直棱恒为铅直；停稳的额外立体感由一层固定在**屏幕空间**的**纯偏航展示偏摆**提供（旋转时淡出、停稳后淡回），逻辑姿态与展示偏摆分离。主面占三个可见面投影面积 71.4%（24 种标准朝向零离散），右面 18.1%、顶面 10.5%。玩法、输入映射、阈值与存档沿用 v0.8.5。口径见 [03 §2.1](docs/Planning/03-交互与表现需求.md)，含被否决的 v0.8.6 一版的验收见 [04](docs/Planning/04-MVP验收清单.md)。

v0.8.6：~~相机改正视、由六面体双轴偏摆提供 3/4 感（主面 82.8%）~~ **已被制作人实测否决并废弃**——正面近似正视会让每一格都成近正方形，画面退化成平面网格，且双轴偏摆把棋盘压歪 −4.77°。改动保留的部分是展示偏摆机制、easeOutCubic 吸附与"存档只存逻辑姿态"。
