...
当前版本 **0.8.4**：**候选池改为加权发牌**——六种四格件（2×2、L、J、T、S、Z）各 2 份权重、四种小块各 1 份，即 75% 四格件 / 25% 小块，**十种形状全部保留**。改动只有 `src/game/shapes.js` 的 `SHAPE_WEIGHTS` + `pickShape()` 与 `src/main.js` 的三处发牌；渲染、存档、开局预置与其余规则与 v0.8.3 相同。动机是"太轻松"（会消除的玩家从不接近危险），依据是 [紧张度测量](docs/Technical/DIFFICULTY_TENSION.md)：典型打法自然结束率 1.0% → 6.5%、一局最紧时刻 9 → 6 个落点。回滚：把四格件权重改回 1。
