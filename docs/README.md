...
> 当前实现基线：**v0.8.16**（以 `package.json` 和源码为准）。本轮为文档静态整理，没有重跑游戏测试、截图、构建或平台联调。
...
- **Planning/**：01～08 按源码描述现行实现；研究、建议和待实施段必须显式标注，不因放在该目录就自动生效。
- **Technical/**：工程事实、具名实验、流程规范和历史验证；文件类型与版本决定用途，不统一视为现行需求。
- **Archive/**：历史原稿，保留旧决策以便溯源，不覆盖现行文档。
- **Temporary/**：本机临时交接资料（当前受 Git 忽略），不是现行规范，也不作为版本库导航的必要依赖。旋转旧稿的 82%～88% 主面目标、短滑一律弹回等已被后续实现取代，以 03/04 为准。
